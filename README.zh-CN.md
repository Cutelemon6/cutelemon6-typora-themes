...
当前分支为审阅草稿。下方截图展示初版效果，扩展 Markdown 格式仍待审阅，详见 [TODO](TODO.md)。

三套主题统一采用 **16 px 正文**、**桌面可用编辑宽度的 75%**和 **28 / 22 / 18 px 标题层级**。通过字体、间距和少量有明确用途的颜色，让页面具有稳定的阅读节奏。
...
中文字体采用中国大陆简体字形规范。字体随主题提供并从本地加载，无需安装到系统字体库。代码沿用内置 Night 的字体栈：Monaco、Consolas、Andale Mono、DejaVu Sans Mono，最后回退到系统等宽字体。macOS 上优先使用可用的 Monaco；这些系统字体不随主题分发。
...
- **宽度：**占可用编辑容器的 75%，包含左右各 32 px 内边距；窗口宽度不超过 1000 px 时自动铺开，打印使用页面宽度。
- **标题：**全部左对齐；只在 H2 下保留细横线；H2 上方 32 px、下方 10 px。
- **段落：**段后 12 px；列表项之间 4 px。
- **链接：**默认显示 1 px 下划线，悬停时加粗；访问前后颜色一致。
- **引用：**淡底、左侧边线、正常直立文字。
- **代码：**13.5 px 等宽字体、1.6 行高。Night 参考 Typora 内置 Night，使用平整的 `#27292d` 背景、紫色关键字、红色字符串、橙色注释与蓝紫色定义名；Day 和 Editorial 保留原有浅色代码样式。行号跟随 Typora「偏好设置 → Markdown → 代码块」中的设置，主题不强制显示或隐藏。
...
:root {
  ...
  --cutelemon6-content-max-width: 75%; /* 包含编辑区左右内边距 */
}
```
...
浏览器预览覆盖基础 Markdown 排版，并使用 CodeMirror 5 为 Python、Shell、JavaScript 和 JSON 分词着色。原生编辑器交互、公式、脚注与最终高亮行为，仍请在 Typora 中使用完整样张检查。
...
**第三方字体保留各自授权，不属于 MIT 范围。** 浏览器预览另包含 CodeMirror 5.65.16，并保留其自身的 MIT 声明；安装到 Typora 的主题不依赖这部分文件。思源黑体、思源宋体和 Source Serif 4 使用 SIL OFL 1.1；随包 Open Sans 版本使用 Apache 2.0。仓库直接提供上游原版字体二进制文件，并保留版权声明和许可。具体来源与 SHA-256 校验值见[第三方说明](THIRD_PARTY_NOTICES.md)和[字体清单](font-manifest.json)。
